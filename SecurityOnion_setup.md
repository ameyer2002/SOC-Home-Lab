...
What I did from here was boot my pfsense machine which contains the LAN of all my machines. From the security onion terminal, I tried pinging the kali and machine but didn't get any ICMP messages sent back to me. I then went into each of those machines as ran a **ip a** on the kali machine and **ipconfig** on the windows machine to confirm the IP addresses were those from the IP range from the LAN. I saw they weren't but then realized my pfsense machine would have to be the first machine started in this lab in order for every other machine to have the correct IP in the LAN.
